...
- Начни первым: "${clientName}, здравствуйте. Удобно сейчас говорить?"
...
- Если клиент уже явно теплый, не продолжай продавать и не раскрывай лишних деталей: уточни недостающее одним коротким вопросом или заверши.
...
- Если клиент заинтересован, задай 2-4 коротких уточняющих вопроса и заверши.
...
function cleanLead(input, userId) {
  ...
  return {
    ...
    status: statuses.has(input.status) ? input.status : "unclear",
    interest_score: Number.isFinite(score) ? Math.max(0, Math.min(100, Math.round(score))) : 0,
    deal_type: String(input.deal_type || "неизвестно").trim().slice(0, 160),
