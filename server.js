...
- Как только клиент сказал, что хочет купить, арендовать или продать недвижимость, он уже теплый.
- После первого явного интереса можно задать максимум один короткий уточняющий вопрос для вида.
- Если клиент ответил на этот вопрос, отказался уточнять или начал раздражаться, сразу завершай звонок теплым статусом.
...
- Если клиент уже явно теплый, не продолжай расспрашивать: один короткий вопрос максимум, затем финальная фраза.
...
- Если клиент сначала проявил интерес, а потом раздражается из-за вопросов, это все равно теплый клиент. Причина: интерес был, детализацию не захотел.
...
function cleanLead(input, userId) {
  const statuses = new Set(["warm", "cold", "unclear"]);
  ...
  const status = statuses.has(input.status) ? input.status : "unclear";
  const normalizedStatus = hasRealEstateIntent && status === "cold" ? "warm" : status;
  const normalizedScore =
    normalizedStatus === "warm" && Number.isFinite(score) && score < 60 ? 60 : score;
